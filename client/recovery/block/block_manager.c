...
int register_block_manager(struct block_manager* this) {
    ...
    list_for_each(cell, &block_manager_list) {
        ...
        if (!strcmp(m->name,  this->name)) {
            LOGE("Block manager \''%s\' is already registered", m->name);
            return false;
        }
    }
...
int unregister_block_manager(struct block_manager* this) {
    ...
    list_for_each(cell, &block_manager_list) {
        m = list_entry(cell, struct block_manager, list_cell);
        if (!strcmp(m->name,  this->name)) {
            LOGI("Block manager \''%s\' is removed successfully", m->name);
            list_del(cell);
            return true;
        }
    }
...
void get_supported_block_managers(struct block_manager* this, char *buf) {
    struct block_manager *m;
    struct list_head *cell;
    char list[128];
    int i = 0;
    ...
    strcat(list, "]");
    strcpy(buf, list);
    LOGI("Block manager supporting list \''%s\'",  buf);
    return;
}
...
void get_supported_filetype(struct block_manager* this, char *buf) {
    ...
    strcat(list, "]");
    strcpy(buf, list);
    LOGI("Block manager supporting filesystem list  \''%s\'",  buf);
    return;
}
