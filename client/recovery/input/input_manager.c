...
static void dump_input_device(struct input_manager* this) {
    struct list_head* pos;

    LOGD("========================================\n");
    LOGD("Dump input devices\n");
    list_for_each(pos, &this->input_dev_list) {
...
static int scan_devices(struct input_manager* this) {
    ...
    for (int i = 0; i < ndev; i++) {
        ...
        list_add_tail(&device->head, &this->input_dev_list);
    }

    return 0;
}

static inline const char* typename(struct input_manager* this,
        unsigned int type) {
    return (type <= EV_MAX && events[type]) ? events[type] : "?";
}

static inline const char* codename(struct input_manager* this,
        unsigned int type, unsigned int code) {
    return (type <= EV_MAX && code <= maxval[type] && names[type] &&
            names[type][code]) ? names[type][code] : "?";
}

static void dump_event(struct input_manager* this, struct input_event* event) {
    LOGI("========================================\n");
    LOGI("Dump input event\n");
    LOGI("time:  %ld.%06ld\n", event->time.tv_sec, event->time.tv_usec);
    LOGI("type:  %s\n", typename(this, event->type));
    LOGI("code:  %s\n", codename(this, event->type, event->code));
    LOGI("value: 0x%x\n", event->value);
    LOGI("========================================\n");
}

static void on_event(struct input_manager* this, struct input_event* event) {
    struct list_head* pos;

    pthread_mutex_lock(&this->listener_lock);

    list_for_each(pos, &this->listener_list) {
...
static void* thread_loop(void *param) {
    ...
    struct input_manager* this = (struct input_manager*)param;

    FD_ZERO(&rdfs);

    if (this->max_fd < this->local_pipe[0])
        this->max_fd = this->local_pipe[0];
    ...
    for (;;) {
        error = select(this->max_fd + 1, &rdfs, NULL, NULL, NULL);
        if (error <= 0) {
        ...
        pthread_mutex_lock(&this->device_list_lock);
        ...
        pthread_mutex_unlock(&this->device_list_lock);

        if (FD_ISSET(this->local_pipe[0], &rdfs)) {
            char c;
            error = read(this->local_pipe[0], &c, 1);
            if (!error) {
...
static int start(struct input_manager* this) {
    int error = 0;

    error = pipe(this->local_pipe);
    if (error) {
    ...
    error = pthread_create(&tid, &attr, thread_loop, (void*)this);
    if (error) {
...
static int stop(struct input_manager* this) {
    char c = 0;

    if (!write(this->local_pipe[1], &c, 1)) {
...
static int init(struct input_manager* this) {
    int error = 0;

    INIT_LIST_HEAD(&this->input_dev_list);
    INIT_LIST_HEAD(&this->listener_list);
    pthread_mutex_init(&this->device_list_lock, NULL);
    pthread_mutex_init(&this->listener_lock, NULL);

    error = scan_devices(this);
    if (error < 0) {
        LOGE("Failed to scan input device\n");
        return -1;
    }

    dump_input_device(this);

    return 0;
}

static int deinit(struct input_manager* this) {
    pthread_mutex_lock(&this->device_list_lock);
    struct list_head* pos;
    struct list_head* next_pos;
    list_for_each_safe(pos, next_pos, &this->input_dev_list) {
        struct input_device* device = list_entry(pos, struct input_device, head);
    ...
    if (this->local_pipe[0] > 0)
        close(this->local_pipe[0]);

    if (this->local_pipe[1] > 0)
        close(this->local_pipe[1]);

    pthread_mutex_destroy(&this->device_list_lock);
    pthread_mutex_destroy(&this->listener_lock);

    return 0;
}

static int get_device_count(struct input_manager* this) {
    pthread_mutex_lock(&this->device_list_lock);

    int size = get_list_size_locked(&this->input_dev_list);
...
static void register_event_listener(struct input_manager* this,
    ...
    l->cb = listener;
    list_add_tail(&l->head, &this->listener_list);

    pthread_mutex_unlock(&this->listener_lock);
}

static void unregister_event_listener(struct input_manager* this,
        input_event_listener_t listener) {
    assert_die_if(listener == NULL, "listener is NULL\n");
    ...
    list_for_each_safe(pos, next_pos, &this->listener_list) {
        struct listener* l = list_entry(pos, struct listener, head);
...
void construct_input_manager(struct input_manager* this) {
    ...
    this->start = start;
    this->stop = stop;
    this->get_device_count = get_device_count;
    this->register_event_listener = register_event_listener;
    this->unregister_event_listener = unregister_event_listener;
    this->type2str = typename;
    this->code2str = codename;
...
void destruct_input_manager(struct input_manager* this) {
    ...
    this->start = NULL;
    this->stop = NULL;
    this->get_device_count = NULL;
    this->register_event_listener = NULL;
    this->unregister_event_listener = NULL;
    this->type2str = NULL;
    this->code2str = NULL;
    this->dump_event = NULL;
}
