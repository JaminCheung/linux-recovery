...
struct ubi_vtbl_record *ubigen_create_empty_vtbl(const struct ubigen_info *ui)
{
	...
	for (i = 0; i < ui->max_volumes; i++) {
		uint32_t crc = local_crc32(UBI_CRC32_INIT, &vtbl[i],
		      ...
		      struct ubi_vtbl_record *vtbl)
{
	...
	tmp = local_crc32(UBI_CRC32_INIT, vtbl_rec, UBI_VTBL_RECORD_SIZE_CRC);
			 ...
			 const void *data, int data_size)
{
	...
	if (vi->type == UBI_VID_STATIC) {
		...
		crc = local_crc32(UBI_CRC32_INIT, data, data_size);
		hdr->data_crc = cpu_to_be32(crc);
	}

	crc = local_crc32(UBI_CRC32_INIT, hdr, UBI_VID_HDR_SIZE_CRC);
	hdr->hdr_crc = cpu_to_be32(crc);
}
