...
#include <sys/stat.h>
#include <sys/types.h>
#include <lib/libubi.h>
#include <lib/libubi_int.h>
#include <lib/libcommon.h>
#include <utils/log.h>

#define LOG_TAG "recovery--->libubi"
...
static char *mkpath(const char *path, const char *name)
{
	...
	if (!n) {
	    LOGE("cannot allocate %d bytes", len1 + len2 + 2);
		return NULL;
	}
			 ...
			 int *vol_id)
{
	...
	if (stat(node, &st)) {
		LOGE("cannot get information about \"%s\"",
				  node);
		return -1;
	}
	...
	if (minor == 0) {
		errno = EINVAL;
		LOGE("\"%s\" is not a volume character device", node);
		return -1;
	}
...
static int dev_node2num(struct libubi *lib, const char *node, int *dev_num)
{
	...
	if (minor != 0) {
		errno = EINVAL;
		LOGE("\"%s\" is not an UBI character device", node);
		return -1;
	}
	...
	for (i = info.lowest_dev_num; i <= info.highest_dev_num; i++) {
		...
		if (major1 == major) {
			if (minor1 != 0) {
				LOGE("UBI character device minor number is "
...
libubi_t libubi_open(void)
{
	...
	if (close(fd)) {
		LOGE("close failed on \"%s\"", lib->sysfs_ubi);
		goto out_error;
	}
	...
	if (version != LIBUBI_UBI_VERSION) {
	    LOGE("this library was made for UBI version %d, but UBI "
...
static int mtd_node_to_num(const char *mtd_dev_node)
{
	...
	if (stat(mtd_dev_node, &sb) < 0) {
	    LOGE("cannot stat \"%s\"", mtd_dev_node);
	    return -1;
	}

	if (!S_ISCHR(sb.st_mode)) {
		errno = EINVAL;
		LOGE("\"%s\" is not a character device",
				  mtd_dev_node);
		return -1;
	}
	...
	if (major != MTD_CHAR_MAJOR) {
		errno = EINVAL;
		LOGE("\"%s\" is not an MTD device", mtd_dev_node);
		return -1;
	}
...
out_not_ubi:
	LOGE("\"%s\" has major:minor %d:%d, but this does not correspond to "
...
int ubi_get_info(libubi_t desc, struct ubi_info *info)
{
	...
	if (!dirent && errno) {
	    LOGE("readdir failed on \"%s\"", lib->sysfs_ubi);
		goto out_close;
	}

	if (closedir(sysfs_ubi)) {
	    LOGE("closedir failed on \"%s\"", lib->sysfs_ubi);
		return -1;
	}
